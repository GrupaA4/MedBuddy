...
def check_similarity_with_glove(word, words_set=words_set):
    """
    ...
    for word_in_set in words_set:
        ...
        if similarity_score > 0.52:  # JOACA TE CU VALOAREA
            similar_words.append((word_in_set, similarity_score))

    similar_words.sort(key=lambda x: x[1], reverse=True)

    top_similar_words = similar_words[:3]  # most 3 similar words
...
# similar_words = check_similarity_with_glove(word_to_check, words_set=words_set)
...
# doc = nlp("Last night i had shivers, i started coughing 2 days ago extremely hard and since i've met my mother, "
# "i felt dizzy and i have breath shortness regulary i cant eat anymore because im so weak and sometimes i "
# "even have high fever, should i visit a doctor and together with that i vomited a lot?")
